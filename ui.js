...
export function initButtonHovers(){
  ...
  const handleEnter=(btn)=>{ if(btn.disabled) return; stopAll(); btn.classList.add('static-bg-active'); const h=playSound(uiHoverBuffer, 0.2); if(h) initialSoundSource=h.source; staticLoopSound=playSound(tvStaticLoopBuffer, 0.1, null, true, 0.5); };
